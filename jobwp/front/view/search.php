...
$jobwp_categories = get_terms( array(
    'taxonomy'   => 'jobs_category',
    'hide_empty' => true,
    'order'      => 'ASC',
    'parent'     => 0,
) );
$jobwp_types = get_terms( array(
    'taxonomy'   => 'jobs_nature',
    'hide_empty' => true,
    'order'      => 'ASC',
    'parent'     => 0,
) );
$jobwp_locations = get_terms( array(
    'taxonomy'   => 'jobs_location',
    'hide_empty' => true,
    'order'      => 'ASC',
    'parent'     => 0,
...
if ( !$jobwp_hide_search_keyword ) {
    ?>
            <div class="jobwp-search-item">
                <input type="text" name="jobwp_title_s" placeholder="<?php 
    esc_attr_e( $jobwp_search_keyword_ph );
    ?>" value="<?php 
    esc_attr_e( $jobwp_title_s );
    ?>">
            </div>
            <?php 
}
if ( !$jobwp_hide_search_category ) {
    ...
    foreach ( $jobwp_categories as $job_category ) {
        ?>
                        <option value="<?php 
        esc_attr_e( $job_category->name );
        ?>" <?php 
        selected( htmlspecialchars( $jobwp_category_s ), $job_category->name );
        ?> ><?php 
        esc_html_e( $job_category->name );
        ?></option>
                        <?php 
    }
    ?>
...
if ( !$jobwp_hide_search_jobtype ) {
    ...
    foreach ( $jobwp_types as $jobwp_type ) {
        ?>
                        <option value="<?php 
        esc_attr_e( $jobwp_type->name );
        ?>" <?php 
        echo ( $jobwp_type_s == $jobwp_type->name ? 'Selected' : '' );
        ?>><?php 
        esc_html_e( $jobwp_type->name );
        ?></option>
                        <?php 
    }
    ?>
...
if ( !$jobwp_hide_search_location ) {
    ...
    foreach ( $jobwp_locations as $job_location ) {
        ?>
                        <option value="<?php 
        esc_attr_e( $job_location->name );
        ?>" <?php 
        echo ( $jobwp_location_s == $job_location->name ? 'Selected' : '' );
        ?>><?php 
        esc_html_e( $job_location->name );
        ?></option>
                        <?php 
    }
    ?>
...
?>">
        ...
        <div class="jobwp-search-item">
            <a href="<?php 
echo get_permalink( $post->ID );
?>" class="fa fa-refresh" id="jobwp-search-refresh"></a>
        </div>
